const passwords = `9-10 b: bbktbbbxhfbpb
2-10 x: xxnxxxwxxsx
5-7 w: ghwwdrr
4-6 z: nzzjzk
7-8 s: szsssswfs
...
const passwordsArray = passwords.match(/^.*[^\]\n](?:\]\n.*[^\]\n])*$/gm);

//Convert data in array of objects
const passwordsObject = passwordsArray.map((pass) => {
  return Object.assign(
    {},
    pass.replace(/[-]/g, ",").replace(/[:]/g, "").split(" ")
  );
});

//PRIMERA PARTE

//Iterate through so as to check passwords that comply policy

let count = 0;
let correct = [];

let all = passwordsObject.map((password) => {
  let passwordString = password[2].split(password[1]).length - 1;
  let number = parseInt(password[0].split(","));
  let number2 = parseInt(password[0].split(",").pop());

  passwordString >= number && passwordString <= number2
    ? count++ && correct.push(password[2])
    : null;

  //Display the passwords in the browser
  document.getElementById("firstDiv").innerHTML = correct.join("<br/>");
});

//Display the count of the correct passwords in the browser
let element = document.getElementsByTagName("body");
document.write(
  `Number of correct passwords with first policy: ${count}` + "<br>"
);

//SEGUNDA PARTE
let secondCount = 0;
let correctSecondPolicy = [];

//Iterate through so as to check passwords that comply policy
passwordsObject.map((password) => {
  let number = parseInt(password[0].split(","));
  let number2 = parseInt(password[0].split(",").pop());

  password[2].charAt(number - 1) === password[1] &&
  password[2].charAt(number2 - 1) === password[1]
    ? null
    : (password[2].includes(password[1]) &&
        password[2].charAt(number - 1) === password[1]) ||
      password[2].charAt(number2 - 1) === password[1]
    ? secondCount++ && correctSecondPolicy.push(password[2])
    : null;

  //Display the passwords in the browser
  document.getElementById("secondDiv").innerHTML = correctSecondPolicy.join(
    "<br/>"
  );
});
...
document.write(
  `Number of correct passwords with second policy: ${secondCount}`
);

//Toggle password list button
togglediv = (id) => {
